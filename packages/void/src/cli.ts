...
function getLocalDbPath(): string | null {
  ...
  for (const subdir of readdirSync(d1Dir)) {
    ...
    if (dbFile) return join(dir, dbFile as string);
  }
  return null;
}

function writeTempConfig(config: VoidJson, extra: Record<string, unknown> = {}): string {
...
function requireLocalDb(): string {
  ...
  if (!dbPath) {
    console.error("No local D1 database found. Run `vite dev` first to initialize it.");
    process.exit(1);
  }
  return dbPath;
}
...
const generate = defineCommand({
  meta: { name: "generate", description: "Generate SQL migration files from your schema" },
  run() {
...
const migrate = defineCommand({
  meta: { name: "migrate", description: "Push schema changes to local D1 database" },
  run() {
    const config = loadVoidJson();
    const dbPath = requireLocalDb();
    const tempConfig = writeTempConfig(config, { dbCredentials: { url: dbPath } });
...
const studio = defineCommand({
  meta: { name: "studio", description: "Open Drizzle Studio to browse your local database" },
  run() {
    const config = loadVoidJson();
    const dbPath = requireLocalDb();
    const tempConfig = writeTempConfig(config, { dbCredentials: { url: dbPath } });
