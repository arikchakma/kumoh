...
function generateTypes(config: KumohConfig, root: string): void {
  ...
  sections.push(
    ...
    '  export function defineHandler<P extends string = any, E extends Env = KumohEnv, R = any>(',
    '    handler: (c: Context<E, P>) => R',
    '  ): (c: Context<E, P>) => R;',
  ...
  if (rpcChains.length) {
    // Include the kumoh/db schema augmentation so response types resolve
    const schemaRef = config.schemaPath
    ...
    if (schemaRef) {
      rpcLines.push(
        `import type * as _schema from '${schemaRef}';`,
        "declare module 'kumoh/db' { export const schema: typeof _schema; }"
      );
    }
    rpcLines.push(
      ...rpcImports,
